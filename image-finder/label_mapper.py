...
Usage:
  # single lookup
  python label_mapper.py ubuntu-22.04 --provider aws
  python label_mapper.py ubuntu-24.04 --provider gcp
  python label_mapper.py ubuntu-24.04 --provider azure

  # use as a library:
  from label_mapper import map_label
  map_label("ubuntu-22.04")["aws"]   -> "ubuntu-jammy-22.04"
"""
...
# --- Core logic ---
_LABEL_RE = re.compile(r"^ubuntu-(\d{2}\.\d{2})$")

def _parse_label(label: str) -> Optional[str]:
    """
    Given a label like 'ubuntu-22.04', return '22.04'. Return None if invalid.
    """
...
def map_label(label: str) -> Dict[str, str]:
    """
    Map a single generic label (e.g., 'ubuntu-22.04') to provider-specific names.
    ...
    if not version:
        raise ValueError(f"Invalid label format: '{label}'. Expected 'ubuntu-YY.MM'.")
...
def main():
    ...
    p.add_argument("label", nargs="?", help="Label like 'ubuntu-22.04'")
